...
import {searchID_1,searchuserID_1,searchAll_1,deleteID_1,change_1,reject_1} from "../../api/index"
import YuForm from './yuForm'
import YuUpdate from './yuUpdate'


export default class LYuMaoQiu extends Component {
  state = {
    shenqings:[], // 用来存储申请列表
    searchshenqing:[],
    searchusers:[], //
  ...
  getShenqings = async ()=>{
    ...
    const result = await searchAll_1()
  ...
  deleteShenqing= async (shenqing)=> {
    const result = await deleteID_1(shenqing.id)
    this.getShenqings()
    message.info(result)
  }
  ...
  Allow = async (shenqing)=>{
      const result = await change_1(shenqing)
      message.info(result)
      this.getShenqings()
  }

  Reject = async (shenqing)=>{
      const result = await reject_1(shenqing)
      message.info(result)
      this.getShenqings()
  }
  ...
  onFinish =  async (value) => {
    const id = value.id; 
    console.log(id)
    const result = await searchID_1(id)
    console.log(result)
  ...
  onFinish_1 = async (value) =>{
    ...
    var result = await searchuserID_1(userID)
    console.log(result)
