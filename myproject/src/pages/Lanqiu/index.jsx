...
import {searchID,searchuserID,searchAll,deleteID,change,reject} from "../../api/index"
import LanForm from './lanForm'
import LanUpdate from './lanUpdate'

export default class LanQiu extends Component {
  state = {
    shenqings:[], // 用来存储申请列表
    searchshenqing:[],
    searchusers:[], //
  ...
  deleteShenqing= async (shenqing)=> {
    const result = await deleteID(shenqing.id)
    this.getShenqings()
    message.info(result)
  }
  ...
  Allow = async (shenqing)=>{
      const result = await change(shenqing)
      message.info(result)
      this.getShenqings()
  }

  Reject = async (shenqing)=>{
      const result = await reject(shenqing)
      message.info(result)
      this.getShenqings()
  }
  ...
  onFinish_1 = async (value) =>{
    ...
    var result = await searchuserID(userID)
    console.log(result)
